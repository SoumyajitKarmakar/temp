...
class MaskedAutoencoderViT(nn.Module):
    ...
    def forward(self, imgs, mask_ratio=0.75):
        ...
        patch_embed = latent_vanilla[: ,1: , :]
        predicted_class = self.head(cls_vanilla)  # Class predictions by the network

        img_patch_vanilla = latent_vanilla[: , 1: , :]
        img_patch_vanilla = self.decoder_embed_vanilla(img_patch_vanilla)
        #This doesnt have class token
        pred_decoder = self.forward_decoder(latent, ids_restore)
        
        loss_twobranch = self.forward_loss(img_patch_vanilla , pred_decoder , mask)
        return loss_twobranch, predicted_class , patch_embed
